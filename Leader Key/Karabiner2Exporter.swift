...
final class Karabiner2Exporter {
  ...
  static func karabinerTsModuleSource(managedRules: [[String: Any]]) -> String {
    ...
    let rulesJSONLiteral = javascriptStringLiteral(rulesJSONString)
    ...
    return """
      ...
      export const leaderKeyManagedRules = JSON.parse(\(rulesJSONLiteral)) as Rule[]
  ...
  private static func actionSignature(for action: Action) -> String {
    ...
    if let data = try? encoder.encode(action), let string = String(data: data, encoding: .utf8) {
      return string
    }
