...
extension UserConfig {
  ...
  // Gets only the fallback config (app-fallback-config.json), or default if not found
  func getFallbackConfig() -> Group {
    ...
    do {
      ...
      if isDefaultConfig && !suppressAlerts {
        ...
        alertHandler.showAlert(
          style: .warning,
          message:
            "Error decoding app-specific config file \(configName):\n\(errorDesc)\n\nThis config will be ignored."
        )
      }
      return nil
    } catch {
      ...
      print(
        "[UserConfig] Unexpected error processing config \(configName): \(error.localizedDescription)"
      )
      if isDefaultConfig && !suppressAlerts {
        alertHandler.showAlert(
          style: .critical,
    ...
    case .action(var action):
      ...
      if let macroSteps = action.macroSteps {
        action.macroSteps = macroSteps.map { step in
          var newStep = step
          newStep.action =
            markAsFromFallback(.action(newStep.action), fallbackSource: fallbackSource).item
            as! Action
          return newStep
        }
      }
  ...
  private func mergeWithFallback(
    appSpecificGroup: Group, fallbackGroup: Group, fallbackSource: String
  ) -> Group {
    print(
      "[UserConfig] mergeWithFallback: Merging app-specific '\(appSpecificGroup.displayName)' with fallback '\(fallbackGroup.displayName)'"
    )
