...
final class ConfigEditorSession: ObservableObject {
  ...
  func applyCommandScoutSuggestions(_ suggestions: [CommandScoutSuggestion]) -> CommandScoutApplyResult {
    ...
    for suggestion in suggestions {
      ...
      let insertion = CommandScoutTreeInsertion.inserting(
        tokens: tokens,
        action: action,
      ...
      guard insertion.inserted else {
        skippedMessages.append("\(suggestion.title): \(insertionFailureMessage(insertion.failure))")
        continue
      }
      root = insertion.root
      insertedCount += 1
  ...
  private func insertionFailureMessage(_ failure: CommandScoutTreeInsertionFailure?) -> String {
    switch failure {
    case .invalidSequence:
      return "invalid sequence"
    case .blockedPrefix(let path):
      return "blocked prefix \(path.joined())"
    case .occupiedLeaf(let path):
      return "occupied sequence \(path.joined())"
    case nil:
