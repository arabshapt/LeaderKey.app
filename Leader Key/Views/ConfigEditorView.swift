...
struct KeyReference {
  static let keyCategories: [String: [String]] = [
    "Special Commands": [
      "vk_none", "release_modifiers", "delay:500", "keydown:left_command", "keyup:left_command",
      "keydown:left_shift", "keyup:left_shift", "keydown:left_option", "keyup:left_option",
      "keydown:left_control", "keyup:left_control", "keydown:right_command", "keyup:right_command",
      "keydown:right_shift", "keyup:right_shift", "keydown:right_option", "keyup:right_option",
      "keydown:right_control", "keyup:right_control",
    ],
    "Letters": [
    ...
    "Special Keys": [
      "escape", "tab", "spacebar", "return_or_enter", "enter", "delete_or_backspace",
      "delete_forward", "home", "end", "page_up", "page_down", "help", "insert",
    ],
    "Keypad": [
      ...
      "keypad_minus", "keypad_multiply", "keypad_divide", "keypad_equal_sign", "keypad_clear",
      "keypad_num_lock",
    ],
    "Modifiers": [
      "caps_lock", "left_control", "left_shift", "left_option", "left_command", "right_control",
    ...
    "Other": [
      "print_screen", "scroll_lock", "pause", "lang1", "lang2", "japanese_eisuu", "japanese_kana",
    ],
  ]
  ...
  static func getKeyHelp(for key: String) -> String {
    ...
    case "vk_none":
      return
        "Release all modifier keys (Karabiner-compatible). Use in sequences like 'Ctab vk_none'"
    ...
    case let str where str.hasPrefix("delay:"):
      return
        "Wait for specified milliseconds (0-10000). Example: 'tab delay:500 tab' waits 500ms between tabs"
    case let str where str.hasPrefix("keydown:"):
      return
        "Hold down the specified key. Example: 'keydown:left_command tab tab keyup:left_command' for app switching"
...
func makeTrueDuplicate(item: ActionOrGroup) -> ActionOrGroup {
  ...
  case .group(let group):
    ...
    var newGroup = Group(
      key: group.key, label: group.label, iconPath: group.iconPath, stickyMode: group.stickyMode,
      actions: newActions)
    // Preserve metadata properties
    ...
    return AnyView(
      HStack(spacing: generalPadding) {
        ...
        case .shortcut:
            ...
            HStack(spacing: 4) {
              ...
              Text(
                state.valueInputValue.isEmpty
                  ? "Set shortcut…"
                  : (state.valueInputValue.count > 25
                    ? "\(state.valueInputValue.prefix(25))…" : state.valueInputValue))
            }
          }
          .buttonStyle(.plain)
          .sheet(isPresented: $state.isShortcutEditorPresented) {
            VStack(alignment: .leading, spacing: 12) {
              ...
              Text(
                "Use letters for modifiers before the key: C=⌘, S=⇧, O=⌥, T=⌃. Example: CSb means ⌘⇧B."
              )
              .font(.footnote)
          ...
          Button {
            ...
            HStack(spacing: 4) {
              ...
              Text(
                state.valueInputValue.isEmpty
                  ? "Edit text…"
                  : (state.valueInputValue.count > 20
                    ? "\(state.valueInputValue.prefix(20))…" : state.valueInputValue))
            }
          }
          .buttonStyle(.plain)
        ...
        Button(
          role: .none,
          action: {
            ClipboardManager.shared.copyItem(
              .action(action), fromConfig: userConfig.selectedConfigKeyForEditing)
          }
        ) {
      ...
      .background(
        action.isFromFallback ? Color.blue.opacity(0.05) : Color.clear
      )
    ...
    VStack(spacing: generalPadding) {
      HStack(spacing: generalPadding) {
        ...
        Button(
          ...
          action: {
            ClipboardManager.shared.copyItem(
              .group(group), fromConfig: userConfig.selectedConfigKeyForEditing)
          }
        ) {
          Image(systemName: "doc.on.clipboard")
        }
      ...
      .background(
        group.isFromFallback ? Color.blue.opacity(0.05) : Color.clear
      )
            ...
            HStack(spacing: 4) {
              ...
              Text(
                valueInputValue.isEmpty
                  ? "Set shortcut…"
                  : (valueInputValue.count > 25
                    ? "\(valueInputValue.prefix(25))…" : valueInputValue))
            }
          }
          .buttonStyle(.plain)
          ...
          Button {
            ...
            HStack(spacing: 4) {
              ...
              Text(
                valueInputValue.isEmpty
                  ? "Edit text…"
                  : (valueInputValue.count > 20
                    ? "\(valueInputValue.prefix(20))…" : valueInputValue))
            }
          }
          .buttonStyle(.plain)
