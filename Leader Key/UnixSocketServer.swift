...
final class UnixSocketServer {
    ...
    private let keyProcessingQueue = DispatchQueue(label: "com.leaderkey.keyprocessing", qos: .userInteractive)
    ...
    func start() -> Bool {
        ...
        var reuseAddr = 1
        setsockopt(socketHandle, SOL_SOCKET, SO_REUSEADDR, &reuseAddr, socklen_t(MemoryLayout.size(ofValue: reuseAddr)))
    ...
    private func acceptConnection() {
        ...
        guard clientSocket >= 0 else {
            debugLog("[UnixSocketServer] Failed to accept connection: \(String(cString: strerror(errno)))")
            return
        }
