...
extension UserConfig {
  ...
  func saveCurrentlyEditingConfig() {
    print(
      "[SAVE LOG] saveCurrentlyEditingConfig: CALLED for config key: \(selectedConfigKeyForEditing)"
    )
    print(
      "[SAVE LOG] saveCurrentlyEditingConfig: State of currentlyEditingGroup BEFORE sort: \(currentlyEditingGroup)"
    )

    guard let filePath = discoveredConfigFiles[selectedConfigKeyForEditing] else {
      let errorDesc =
        "Could not find file path for selected config key: \(selectedConfigKeyForEditing)"
      print("[SAVE LOG] saveCurrentlyEditingConfig: ERROR - \(errorDesc)")
      handleError(
        NSError(domain: "UserConfig", code: 3, userInfo: [NSLocalizedDescriptionKey: errorDesc]),
        critical: true)
      return
    }
    ...
    if isAppSpecificConfig {
      print(
        "[SAVE LOG] saveCurrentlyEditingConfig: Stripping fallback items from app-specific config.")
    ...
    if isActivelyEditing {
      ...
      print(
        "[SAVE LOG] saveCurrentlyEditingConfig: State of group AFTER sort (finalGroup): \(finalGroup)"
      )
    }
    // -----------------
    ...
    if !errors.isEmpty {
      errors.forEach {
        print(
          "[SAVE LOG] saveCurrentlyEditingConfig: Validation Error - Path: \($0.path), Msg: \($0.message), Type: \($0.type)"
        )
      }
      // --- MODIFIED: Show error and RETURN if validation fails ---
      let errorCount = errors.count
      let errorMsg =
        "Found \(errorCount) validation issue\(errorCount > 1 ? "s" : "") in the '\(selectedConfigKeyForEditing)' configuration. \nPlease fix the issues before saving."
      ...
      print(
        "[SAVE LOG] saveCurrentlyEditingConfig: No validation errors. Proceeding to encode and write."
      )
    }
