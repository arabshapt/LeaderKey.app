...
final class KarabinerExporter {
  ...
  static func saveToFile(_ content: String, format: ExportFormat) -> URL? {
    ...
    panel.nameFieldStringValue = format == .karabinerJSON ? "leader_key_config.json" : "leader_key_config.edn"
    panel.allowedContentTypes = format == .karabinerJSON ? [.json] : [.plainText]
  ...
  private static func generateKarabinerJSON(from config: UserConfig) -> String {
    ...
    let letterManipulators = "abcdefghijklmnopqrstuvwxyz".map { letter in
      ...
      let command = useCLI ? "\(cliPath) key '\(key)'" : "echo 'key \(key)' | nc -U /tmp/leaderkey.sock"
      return """
        {
          "type": "basic",
          "from": { "key_code": "\(key)" },
...
"""
    ...
    let specialKeyManipulators = [
      ...
      let command = useCLI ? "\(cliPath) key '\(karabinerCode)'" : "echo 'key \(karabinerCode)' | nc -U /tmp/leaderkey.sock"
      return """
        {
          "type": "basic",
...
"""
    ...
    let activateCommand = useCLI ? "\(cliPath) activate" : "echo 'activate' | nc -U /tmp/leaderkey.sock"
    let deactivateCommand = useCLI ? "\(cliPath) deactivate" : "echo 'deactivate' | nc -U /tmp/leaderkey.sock"
...
"""
  ...
  private static func generateGokuEDN(from config: UserConfig) -> String {
    ...
    let specialKeyRules = [
      ...
      guard allKeys.contains(key) || allKeys.contains(gokuCode.replacingOccurrences(of: "_or_", with: ""))
      else { return nil }
      let command = useCLI ? "\(cliPath) key '\(gokuCode)'" : "echo 'key \(gokuCode)' | nc -U /tmp/leaderkey.sock"
      return
        "   [:\(gokuCode) [[:shell \"\(command)\"]] [\"leader_mode\" 1]]"
    }.joined(separator: "\n")
    
    // Pre-compute activation/deactivation commands to avoid complex string interpolation
    let activateAppCmd = useCLI ? "\(cliPath) activate app" : "echo 'activate app' | nc -U /tmp/leaderkey.sock"
    let activateGlobalCmd = useCLI ? "\(cliPath) activate global" : "echo 'activate global' | nc -U /tmp/leaderkey.sock"
    let deactivateCmd = useCLI ? "\(cliPath) deactivate" : "echo 'deactivate' | nc -U /tmp/leaderkey.sock"
