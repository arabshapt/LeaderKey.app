...
class OverlayDetector: ObservableObject {
  ...
  func detectAndCacheOverlayState() -> (bundleId: String?, isOverlay: Bool) {
    ...
    print(
      "[OverlayDetector] 💾 Cached detection result: \(result.bundleId ?? "nil"), overlay: \(result.isOverlay)"
    )
    return result
  }
  ...
  func detectFrontmostAppWithOverlay() -> (bundleId: String?, isOverlay: Bool) {
    ...
    {
      print(
        "[OverlayDetector] 📋 Using cached detection result: \(cached.bundleId ?? "nil"), overlay: \(cached.isOverlay)"
      )
      return cached
    }
  ...
  private func detectRaycastOverlay(for app: NSRunningApplication) -> Bool {
    ...
    print(
      "[OverlayDetector] ❌ No Raycast overlay found after checking all \(windows.count) windows")
    return false
  }
  ...
  private func logWindowProperties(_ window: AXUIElement, index: Int) {
    ...
    var posError = AXUIElementCopyAttributeValue(
  ...
  private func isRaycastSearchWindow(_ window: AXUIElement) -> Bool {
    ...
    if titleError == .success, let title = titleRef as? String {
      ...
      titleIndicatesOverlay =
        title.isEmpty || title == "Raycast" || titleLower.contains("raycast")
        || titleLower.contains("search")
        || title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      print(
        "[OverlayDetector] Title analysis: '\(title)' → overlay indicator: \(titleIndicatesOverlay)"
      )
    } else {
      // No title available might indicate overlay
      titleIndicatesOverlay = true
      print(
        "[OverlayDetector] No title available → assuming overlay indicator: \(titleIndicatesOverlay)"
      )
    }
    ...
    if subroleError == .success, let subrole = subroleRef as? String {
      ...
      subroleIndicatesOverlay =
        subrole == "AXFloatingWindow" || subrole == "AXDialog"
        || subrole == "AXSystemFloatingWindow" || subrole == "AXApplicationDialog"
        || subrole.contains("Float") || subrole.contains("Dialog")
      print(
        "[OverlayDetector] Subrole analysis: '\(subrole)' → overlay indicator: \(subroleIndicatesOverlay)"
      )
    } else {
    ...
    if hasSizePositionData {
      ...
      let isReasonableSize =
        windowSize.width >= 150 && windowSize.width <= 1200 && windowSize.height >= 25
      ...
      if let screen = containingScreen {
        ...
        let isNotAtEdges = windowPoint.x > -screenWidth * 0.1 && windowPoint.x < screenWidth * 1.1  // Allow some off-screen tolerance
        ...
        sizePositionIndicatesOverlay =
          isReasonableSize && isNotAtBottom && isNotAtEdges && isOverlayHeight
        ...
        print(
          "[OverlayDetector]   Overall size/position indicator: \(sizePositionIndicatesOverlay)")
      }
    } else {
    ...
    let fallbackIndicator =
      sizePositionIndicatesOverlay && (titleError != .success || subroleError != .success)
    ...
    if !hasSizePositionData {
      ...
      noSizePositionBoost =
        (titleIndicatesOverlay && subroleIndicatesOverlay)
        || (subroleIndicatesOverlay
          && (actualSubrole == "AXSystemDialog" || actualSubrole == "AXFloatingWindow"))
      print(
        "[OverlayDetector] No size/position data available, using enhanced title/subrole logic: \(noSizePositionBoost)"
      )
    }

    let isOverlay =
      strongIndicator || moderateIndicator || weakIndicator || fallbackIndicator
      || noSizePositionBoost
    ...
    print(
      "[OverlayDetector]   Strong: \(strongIndicator), Moderate: \(moderateIndicator), Weak: \(weakIndicator), Fallback: \(fallbackIndicator), NoSizeBoost: \(noSizePositionBoost)"
    )
  ...
  private func isAlfredSearchWindow(_ window: AXUIElement) -> Bool {
    ...
    if titleError == .success, let title = titleRef as? String {
      ...
      let titleLower = title.lowercased()
      titleIndicatesOverlay =
        title.isEmpty || title == "Alfred" || titleLower.contains("alfred")
        || titleLower.contains("search")
        || title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      print(
        "[OverlayDetector] Alfred title analysis: '\(title)' → overlay indicator: \(titleIndicatesOverlay)"
      )
    } else {
      // No title available might indicate overlay
      titleIndicatesOverlay = true
    ...
    if subroleError == .success, let subrole = subroleRef as? String {
      ...
      subroleIndicatesOverlay =
        subrole == "AXFloatingWindow" || subrole == "AXDialog"
        || subrole == "AXSystemFloatingWindow" || subrole == "AXApplicationDialog"
        || subrole == "AXSystemDialog" || subrole.contains("Float") || subrole.contains("Dialog")
      print(
        "[OverlayDetector] Alfred subrole analysis: '\(subrole)' → overlay indicator: \(subroleIndicatesOverlay)"
      )
    } else {
      print("[OverlayDetector] No subrole available")
    }
    ...
    if hasSizePositionData {
      ...
      let isReasonableSize =
        windowSize.width >= 150 && windowSize.width <= 1000 && windowSize.height >= 25
  ...
  private func isGenericOverlayWindow(_ window: AXUIElement) -> Bool {
    ...
    let isFloatingSubrole =
      props.subrole == "AXFloatingWindow" || props.subrole == "AXSystemFloatingWindow"
    ...
    print(
      "[OverlayDetector] Generic overlay: title='\(props.title)', subrole='\(props.subrole)' → \(isOverlay)"
    )
    return isOverlay
  }
  ...
  func startContinuousTesting() {
    ...
    continuousTestingTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) {
      [weak self] _ in
  ...
  func startRealtimeDetection() {
    ...
    realtimeDetectionTimer = Timer.scheduledTimer(withTimeInterval: 2.5, repeats: true) {
      [weak self] _ in
